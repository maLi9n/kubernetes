...
helm install --namespace=traefik traefik traefik/traefik -f ~/Helm/Traefik/values.yaml --version v24.0.0
...
# Step 12: Apply staging certificate issuer (technically you should use the staging to test as per documentation)
